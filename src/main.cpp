...
HAButton cancelCalibrateButton("sliding_gate_calibrate_cancel");
...
void load_params()
{
    ...
    else
    {
        ...
        if (old_open != 0 || old_close != 0 || old_stop != 0)
        {
            ...
            if (old_open != 0)
                rfKeyList.push_back({old_open, 0});
            if (old_close != 0)
                rfKeyList.push_back({old_close, 1});
            if (old_stop != 0)
                rfKeyList.push_back({old_stop, 2});
            if (old_pos50 != 0)
                rfKeyList.push_back({old_pos50, 3});
...
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
{
    ...
    case WStype_TEXT:
        ...
        if (text == "OPEN")
            ...
            send_command(CMD_CALIBRATE_CANCEL);
        else if (text == "SCAN_START")
            send_command_full(CMD_RF_SCAN_MODE, -1, 0, 1);
        else if (text == "SCAN_STOP")
            send_command_full(CMD_RF_SCAN_MODE, -1, 0, 0);
...
void broadcastStatus()
{
    if (webSocket.connectedClients() > 0)
    {
        ...
        {
            status = "CALIBRATING";
            if (cal_state == CAL_HOMING_CLOSE)
                subStatus = "Homing...";
            else if (cal_state == CAL_MEASURING_OPEN)
                subStatus = "Measuring...";
            else if (cal_state == CAL_VERIFYING_CLOSE)
                subStatus = "Verifying...";
        }
...
void handleRoot()
{
    ...
    html += "<button onclick=\"send('CALIBRATE_CANCEL')\" class='btn red' style='width:45%'>Cancel</button>";

    html += "<br><br><a href='/config'><button class='btn gry' style='width:45%'>WiFi/MQTT</button></a><br><br>";
...
void onCancelCalibrateCommand(HAButton *sender) { send_command(CMD_CALIBRATE_CANCEL); } // [NEW] Implementation
...
void setup()
{
    ...
    cancelCalibrateButton.setName("Cancel Calibration");
    cancelCalibrateButton.setIcon("mdi:cancel");
    cancelCalibrateButton.onCommand(onCancelCalibrateCommand); // [NEW] Link callback
